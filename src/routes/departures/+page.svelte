    ...
    if (selectedStation) {
      try {
        ...
        const departuresWithTime = nextDeparturesRes.map((departure: any) => {
          ...
          const timeUntilDeparture = Math.floor(
            (scheduledDepartureTime - currentTime) / 60000
          ); // Convert to minutes
  ...
  const onStationSelection = async (
    ...
    try {
      ...
      selectedStation = await allStations.find(
        (station: App.Station) =>
          station.stopId.toString() === event.detail.value
      );
...
<div>
  <Header />
  <div class={isSmallScreen ? "flex flex-col items-center" : ""}>
    {#if userId}
      <div class="pt-[2rem] {isSmallScreen ? "flex" : "pl-[4rem]"}">
      ...
      <div
        class="table-container py-[2rem] {isSmallScreen
          ? 'px-[0.7rem]'
          : 'px-[4rem]'}"
      >
        {#if departureRoutes}
            ...
            {#if !loading}
              {#each departureRoutes as route}
                {#if nextDepartures.filter((dep) => dep.direction_id === route.direction_id).length > 0}
                  <tbody>
                    <tr>
                      <td class="flex flex-col text-center justify-center"
                        ><span class="mt-3">{route.direction_name}</span></td
                      >
                      {#each nextDepartures
                        .filter((dep) => dep.direction_id === route.direction_id)
                        .sort((a, b) => a.time_until_departure - b.time_until_departure)
                        .slice(0, isSmallScreen ? 2 : undefined) as departure}
                        <td>
                          <span class="flex flex-col text-center">
                            {departure.time_until_departure !== 0
                              ? `${departure.time_until_departure} min`
                              : "Now"}
                            <p>
                              Platform: {departure.platform_number === null
                                ? "Not Found"
